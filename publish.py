...
from version_forge.version import get_version_string, update_version_universally
...
def verify_package_integrity() -> bool:
    ...
    try:
        ...
        if (
            'name = "ollama-forge"' in pyproject
            and "name = ollama_forge" not in setupcfg
        ):
...
def main() -> None:
    ...
    if changed.returncode != 0:
        confirm = (
            input("📝 Uncommitted changes detected. Bump version? [y/N]: ")
            .strip()
            .lower()
        )
        if confirm == "y":
            ...
            if new_version == current_version:
                print(
                    "❗ New version is the same as current version. No change needed."
                )
            else:
                update_version_universally(new_version)

                changed_after = subprocess.run(
    ...
    if not verify_package_integrity():
        if (
            input("❌ Package integrity checks failed. Continue anyway? [y/N]: ")
            .strip()
            .lower()
            != "y"
        ):
